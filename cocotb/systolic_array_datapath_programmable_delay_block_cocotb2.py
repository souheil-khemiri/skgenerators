...
def display_control_values(dut, height, width):
    ...
    dut._log.info("input_enable:\n%s", input_enable)
    dut._log.info("acc_enable:\n%s", acc_enable)
    dut._log.info("sel_adder_mux:\n%s", sel_adder_mux)
    dut._log.info("sel_acc_mux:\n%s", sel_acc_mux)
...
def compute_control_matrices(cycle, height, width, m, delay_a, delay_b, pipeline_offset=0):
    """Compute per-PE control matrices based on delay-aligned k timing."""
    input_enable = np.zeros((height, width), dtype=np.int32)
    acc_enable = np.zeros((height, width), dtype=np.int32)
    sel_adder_mux = np.zeros((height, width), dtype=np.int32)
    sel_acc_mux = np.zeros((height, width), dtype=np.int32)

    for r in range(height):
        for c in range(width):
            k_a = cycle - int(delay_a[r]) - pipeline_offset - c
            k_b = cycle - int(delay_b[c]) - pipeline_offset - r
            if k_a == k_b and 0 <= k_a < m:
                input_enable[r, c] = 1
                acc_enable[r, c] = 1
                sel_acc_mux[r, c] = 1
                if k_a > 0:
...
async def test_matrix_multiply_with_input_delays(dut):
    ...
    max_delay_a = max(sel_a)
    max_delay_b = max(sel_b)
    pipeline_offset = 0
    max_skew = max(max_delay_a + (WIDTH - 1), max_delay_b + (HEIGHT - 1))
    TOTAL_CYCLES = (M - 1) + max_skew + pipeline_offset + 1
    set_standby_hold_mode(dut,HEIGHT, WIDTH)
    await awaitclok(dut)
    ...
    for i in range(TOTAL_CYCLES):
        ...
        input_enable, acc_enable, sel_adder_mux, sel_acc_mux = compute_control_matrices(
            i, HEIGHT, WIDTH, M, sel_a, sel_b, pipeline_offset
        )
        dut.input_enable.value = pack_matrix_unsigned(input_enable, 1)
        dut.acc_enable.value = pack_matrix_unsigned(acc_enable, 1)
        dut.sel_adder_mux.value = pack_matrix_unsigned(sel_adder_mux, 1)
        dut.sel_acc_mux.value = pack_matrix_unsigned(sel_acc_mux, 1)

        dut._log.info(f"*********************just BEFORE clock edge number:##{i}##*********************")
        #display_control_values(dut, HEIGHT, WIDTH)
        dut._log.info(f"a_vec: {a_vec}")
