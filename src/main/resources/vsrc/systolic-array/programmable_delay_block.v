...
reg [ELEMENT_INPUT_WIDTH-1:0] delay_block [0:HEIGHT-1][0:DEPTH-1];
...
always @(posedge clk) begin
    for (j = 0; j<HEIGHT ;j=j+1 ) begin
        ...
        delay_block[j][0] <= D_unpacked[j];
            for(i_a = 1; i_a < DEPTH; i_a = i_a + 1) begin
...
always @(*) begin
    for (i = 0; i < HEIGHT; i = i + 1) begin
        ...
        end else if(0<sel_delay_unpacked[i] && sel_delay_unpacked[i]<DEPTH) begin
        /* verilator lint_on WIDTHEXPAND */
            Q_unpacked[i] = delay_block[i][sel_delay_unpacked[i]];
